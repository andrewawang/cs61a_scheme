...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(first,expr.second.map(lambda expr: scheme_eval(expr,env)),env)
        # END PROBLEM 5
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** REPLACE THIS LINE ***"
        expr=operands.map(lambda operands:scheme_eval(operands,env))\

        return scheme_apply(self,expr,env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        var=Pair(bindings.first.first,var)
        val=Pair(scheme_eval(bindings.first.second.first,env),val)
        bindings=bindings.second

    check_formals(var)
    return env.make_child_frame(var,val)
    # END PROBLEM 15
...
─────────────██▀─▀██─█──█───█
    ...
    "*** REPLACE THIS LINE ***"
    def make_call_frame(self, args, env):
        #mu_frame=self.env.make_child_frame(self.formals, args)

        #formal=self.formals
        while args is not nil:
            env.define(self.formals.first,args.first)
            self.formals,args=self.formals.second,args.second
        return env
        """
        formal=self.formals
        while args is not nil:
            env.define(formal.first,args.first)
            formal,args=formal.second,args.second
        return env
        """
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            check_procedure(scheme_optimized_eval(expr.first,env))

            result=scheme_apply(scheme_optimized_eval(expr.first,env),expr.second.map(helper),env)
            # END Extra Credit
    return result
"""
to-do: change scheme_eval to include (, , True)
"""
################################################################
